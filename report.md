...
This study compared four CNN architectures — MobileNetV3-Small, InceptionV3, VGG16, and VGG19 — for 16-class embryo developmental phase classification using the custom Embryo Composite Loss (ECL).

**VGG16 achieved the best overall performance** with 57.40% test accuracy and macro F1 of 0.4549, followed closely by VGG19 (55.56%, 0.4269). The larger VGG architectures benefited from their deep feature extractors despite the frozen backbone setting. InceptionV3's multi-scale receptive fields gave it an edge over MobileNetV3-Small (51.95% vs 49.95%), while remaining far more parameter-efficient than VGG16 (21.8M vs 121.7M parameters).

The ECL loss function proved effective across all architectures — particularly for reducing stage boundary violations and improving recall on rare classes such as pPNf and pSB. However, pHB (only 97 total frames; 59 in test) remained undetected by all models, highlighting the fundamental limit of extremely rare classes at current dataset scale.
...
- Rare class recall (p3, p5, p6, p7) remains a challenge across all models due to 526:1 imbalance
